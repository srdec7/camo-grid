...
export function playWinSound() {
  ...
  arpNotes.forEach(({ freq, t }) => {
    playTone(freq,       "triangle", 0.35, now + t, 0.20, c);
    playTone(freq * 2,   "sine",     0.15, now + t, 0.04, c); // octave shimmer
  });
  ...
  chordNotes.forEach((freq, i) => {
    playTone(freq, "triangle", 0.70, now + 0.40, 0.16 - i * 0.03, c);
    playTone(freq, "sine",     0.70, now + 0.40, 0.08 - i * 0.01, c, 8); // slight detune for warmth
  });

  // ── Layer 3: Sparkle shimmer (very high triangle — glittery highs)
  const sparkle = [2093, 2637, 3136]; // C7 E7 G7
  sparkle.forEach((freq, i) => {
    playTone(freq, "triangle", 0.12, now + 0.42 + i * 0.04, 0.05, c);
  });

  // ── Layer 4: Low bass punch at the start for impact
  playTone(130.81, "triangle", 0.25, now, 0.18, c); // C3 bass
  playTone(261.63, "triangle", 0.20, now + 0.27, 0.12, c); // C4 mid bass at peak
}
...
export function playFailSound() {
  ...
  wahs.forEach(({ start, end, t, dur }) => {
    ...
    playGlide(start, end, "sawtooth", now + t, dur, 0.17, c);
    // Sub-octave harmony (triangle — adds body)
    playGlide(start / 2, end / 2, "triangle", now + t, dur, 0.09, c);
  });

  // Final very low groan — seals the defeat
  playGlide(200, 80, "sawtooth", now + 1.22, 0.55, 0.10, c);
}
...
export function playCoinSound() {
  ...
  notes.forEach((freq, i) => {
    playTone(freq, "triangle", 0.18, now + i * 0.07, 0.18, c);
  });
}
...
export function playHeartSound() {
  ...
  [330, 415, 494].forEach((freq, i) => {
    playTone(freq, "sine", 0.35, now + i * 0.04, 0.12, c);
  });
}
...
export function playTickSound() {
  ...
  if (!c) return;
  const now = c.currentTime;
  playTone(880, "square", 0.04, now, 0.06, c);
}
